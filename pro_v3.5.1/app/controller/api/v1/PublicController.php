...
class PublicController
{
    ...
    public function menu_user(Request $request)
    {
        ...
        $userService = $invoiceStatus = $deliveryUser = $isUserPromoter = $userVerifyStatus = $userOrder = $isStaff = $isDelivery = $isChannel = $isPromoterApply = true;
        $levelStatus = true;
        ...
        if ($uid && $userInfo) {
            ...
            $isPromoterApply = app()->make(PromoterApplyServices::class)->count(['uid' => $uid, 'status' => 1, 'is_del' => 0]);
        }
        $auth = [];
        $auth['/pages/users/user_vip/index'] = !$vipOpen;
        $auth['/pages/merchant/apply/index'] = $isChannel;
        $auth['/pages/users/user_spread_user/index'] = !$brokerageFuncStatus || $uid == 0 || !$userDivisionStatus;
        ...
        $auth['/pages/users/distributor/apply'] = !$brokerageFuncStatus || $isUserPromoter || $uid == 0 || !$storeBrokerageApply || $isPromoterApply;
