...
//        }
    ...
    {
        [$uni, $paytype, $from, $quitUrl, $useBalance] = $request->postMore([
            ['uni', ''],
            ['paytype', 'weixin'],
            ['from', 'weixin'],
            ['quitUrl', ''],
        ...
        if ($useBalance == 1) {
            if ($userInfo['now_money'] >= $order['pay_price']) {
                $yue_price = $order['pay_price'];
                $online_price = 0;
                $paytype = PayServices::YUE_PAY;
            } else {
                $yue_price = $userInfo['now_money'];
                $online_price = bcsub($order['pay_price'], $userInfo['now_money'], 2);
            }
        } else {
            $yue_price = 0;
            $online_price = $order['pay_price'];
        }
        $order['pay_price'] = $online_price;
        $order['online_price'] = $online_price;
        $order['yue_price'] = $yue_price;
        $this->services->update(['id' => $order['id']], ['online_price' => $online_price, 'yue_price' => $yue_price]);
...
//            }
                app()->make(StockRecordServices::class)->update(['after_sale_no' => $old_order_id], ['after_sale_no' => $order['order_id']]);
            }
            ...
            switch ($order['pay_type']) {
                case PayServices::WEIXIN_PAY:
                    ...
                    if ($from == 'weixinh5') {
                        return app('json')->status('wechat_h5_pay', ['jsConfig' => $jsConfig, 'order_id' => $order['order_id'], 'online_price' => $online_price]);
                    } elseif ($from == 'weixin' || $from == 'routine') {
                        return app('json')->status('wechat_pay', ['jsConfig' => $jsConfig, 'order_id' => $order['order_id'], 'online_price' => $online_price]);
                    } elseif ($from == 'pc') {
                        return app('json')->status('wechat_pc_pay', ['jsConfig' => $jsConfig, 'order_id' => $order['order_id'], 'online_price' => $online_price]);
                    }
                    break;
                case PayServices::ALIPAY_PAY:
                    ...
                    return app('json')->status(PayServices::ALIPAY_PAY . '_pay', '订单创建成功', ['jsConfig' => $jsConfig, 'order_id' => $order['order_id'], 'pay_key' => $payKey, 'online_price' => $online_price]);
                    break;
...
//                    break;
    ...
    public function lst(Request $request)
    {
        ...
        $field = ['id', 'type', 'pid', 'order_id', 'uid', 'spread_uid', 'pink_id', 'store_id', 'supplier_id', 'shipping_type', 'delivery_type', 'paid', 'pay_type', 'pay_price', 'pay_integral', 'total_num', 'add_time', 'pay_time', 'status', 'refund_status', 'is_del', 'is_send_gift', 'receive_gift_uid', 'online_price'];
    ...
    public function detail(Request $request, StoreOrderEconomizeServices $services, StoreOrderPromotionsServices $storeOrderPromotiosServices, $uni)
    {
        ...
        //如果是未支付,把余额支付和线上支付金额改成 0
        if (!$order['paid'] && $order['pay_type'] != PayServices::OFFLINE_PAY) {
            $order['yue_price'] = 0;
            $order['online_price'] = 0;
            $this->services->update($order['id'], ['yue_price' => 0, 'online_price' => 0]);
        }
