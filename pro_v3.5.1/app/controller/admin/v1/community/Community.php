...
class Community extends AuthController
{
    ...
    public function save($id)
    {
        ...
        $is_admin = true;
        if ($id) {
            $communityInfo = $this->services->getCommunityInfo($id);
            if ($communityInfo['type'] != 0) $is_admin = false;
            $data['relation_id'] = $communityInfo['relation_id'];
        }
        $this->services->saveData($data, $id, $is_admin);
