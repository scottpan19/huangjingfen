...
//        if ($infoDiy && $infoDiy['is_diy']) {
    ...
    {
        if (!(int)$id) return $this->fail('参数错误');
...
//        }
    ...
    {
        $colorData = $this->request->postMore([
            ['aux_color', ''],
            ['gradient_color', ''],
            ['light_color', ''],
            ['theme_color', ''],
        ...
        if (!$status) return $this->fail('参数错误');
        $info = $this->services->get(['template_name' => $type, 'type' => 3]);
        if ($info) {
            $info->value = $status;
            $info->color_data = json_encode($colorData);
            $info->update_time = time();
            $info->save();
            event('diy.update');
    ...
    {
        $colorData = $this->services->getColorChange((string)$type);
        $status = (int)$colorData['value'];
        $color_data = json_decode($colorData['color_data'], true);
        return $this->success(compact('status', 'color_data'));
    }
