...
trait Order
{
    ...
    public function getUpdateInfo(StoreOrderCartInfoServices $cartInfoServices, $id)
    {
        ...
        $priceData = [
            'usedIntegral' => $order['use_integral'],
            'deduction_price' => $order['deduction_price'],
            'first_order_price' => $order['first_order_price']
        ];
        if ($cartInfo && $priceData) {
            [$cartInfo, $spread_ids] = app()->make(StoreOrderCreateServices::class)->computeOrderProductTruePrice($order, $cartInfo, $priceData, $order['uid'], []);
        }
        $result = [
            'orderInfo' => [
                'id' => $order['id'],
...
//                $services->storeProductOrderRefundYFasle((int)$id, $refund_price);
    ...
    public function open_order_refund(Request $request, StoreOrderRefundServices $services, $id)
    {
        ...
        if ($type == 2) {
            ...
            $refund_data['pay_price'] = $order['pay_price'];
            $refund_data['refund_price'] = $refund_price;
//            $refund_data['refunded_price'] = $refund_price;
            if ($order['refund_price'] > 0) {
                mt_srand();
                $refund_data['refund_id'] = $order['order_id'] . rand(100, 999);
            }
            ...
            if ($services->agreeRefund($refundId, $refund_data)) {
                $data['refunded_price'] = $refund_price;
                //主动退款清楚原本退款单
//                $services->delete(['store_order_id' => $id]);
//                if ($data['is_split_order']) {
