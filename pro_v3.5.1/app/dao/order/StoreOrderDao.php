...
class StoreOrderDao extends BaseDao
{
     ...
     * @var string[]
     */
    protected array $withField = ['uid', 'order_id', 'real_name', 'user_phone', 'title', 'total_num', 'delivery_id'];
        ...
        return parent::search($where, $search)->when($isDel, function ($query) use ($where) {
            ...
            switch ($where['pay_type']) {
                ...
                case 5:
                    ...
                    break;
                case 6:
                    $query->where('pay_type', 'weixin')->where('yue_price', '>', 0);
                    break;
                case 7:
                    $query->where('pay_type', 'alipay')->where('yue_price', '>', 0);
                    break;
                case 8:
                    $query->where('pay_type', 'offline')->where('yue_price', '>', 0);
                    break;
            }
            ...
            $query->where(function ($que) use ($where) {
                $que->whereLike('order_id|real_name|user_phone|delivery_id', '%' . $where['real_name'] . '%')
     ...
     * @param string $datebefor 开始日期
     * @param string $dateafter 结束日期
     * @param string $timeType 时间类型(week/month/year/30)
     * @return array
     */
