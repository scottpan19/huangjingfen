...
//            }
            foreach ($result['value'] as $k => $v) {
                ...
                foreach ($attrInfo as $attrKey => $attrItem) {
                    if (implode(',', $result['value'][$k]['attr_arr']) == $attrKey) {
                        ...
                        $result['value'][$k]['vip_price'] = $attrItem['vip_price'];
                        $result['value'][$k]['brokerage'] = $attrItem['brokerage'];
                        $result['value'][$k]['brokerage_two'] = $attrItem['brokerage_two'];
                        $result['value'][$k]['level_price'] = $attrItem['level_price'] != 0 ? json_decode($attrItem['level_price'], true) : [];
                    }
                }
            }
            ...
            $productInfo['attr'] = [
                ...
                'virtual_list' => $virtualService->getArr(isset($result['unique']) ? $result['unique'] :  '', $id),
                ...
                'level_price' => $result['level_price'] != 0 ? json_decode($result['level_price'], true) : [],
            ];
        }
...
//            $item['stock'] = isset($item['is_show']) && $item['is_show'] == 1 ? $item['stock'] : 0;
            ...
            if (((int)$item['brokerage'] + (int)$item['brokerage_two']) > $item['price']) {
        ...
        if ($data['spec_type'] == 1) {
            ...
            if (!empty($defaultItems)) {
                ...
                $settle_price_arr = array_column($filtered, 'settle_price') ?? [];
...
//                $data['settle_price'] = $data['settle_price'] ?? '0.00';
        ...
        if ($data['product_type'] == 0) {
            $inventoryData = $skuList['inventoryData'] ?? [];
            $outInventory = $inInventory = [];
...
//        unset($storeInfo['description']);
        ...
        $storeInfo['stock'] = max($storeInfo['stock'], 0);
     ...
     */
    public function setChannelPrice($price, int $uid, $channel_level_info = [])
    {
        //渠道优惠后价格
        $truePrice = $price;
        //渠道优惠
        ...
        if ($channel_level_info['channel_type'] == 1) {
            $discount = $discount['discount'] ?: 100;
            $truePrice = bcmul((string)$discount, (string)$price, 2);
        } else {
            $channel_level_info = json_decode($channel_level_info['channel_level_price'], true);
            if ($channel_level_info) {
                foreach ($channel_level_info as $val) {
                    if ($discount['channel_type'] == $val['id']) {
                        $truePrice = $val['price'];
                    }
                }
            }
    ...
    public function decProductStock(int $num, int $productId, string $unique = '')
    {
        ...
        if ($unique) {
            ...
            $skuValueServices->decMarketingProductStock($productId, $unique, $num);
        }
    ...
    public function otherInfo(int $id, int $type)
    {
        ...
        $info = $this->dao->get($id, ['id', 'is_brokerage', 'brokerage_type', 'is_sub', 'is_vip', 'level_type', 'channel_type']);
        if (!$info) throw new AdminException('商品不存在');
        $data = [];
        $info['level_type'] = $info['level_type'] ?: 1;
        $info['channel_type'] = $info['channel_type'] ?: 1;
        $data['storeInfo'] = $info->toArray();
        $data['level_list'] = [];
        $data['channel_level_list'] = [];
        if ($type == 1) {
            $data['store_brokerage_ratio'] = bcdiv(sys_config('store_brokerage_ratio', 0), 100, 4);
            $data['store_brokerage_two'] = bcdiv(sys_config('store_brokerage_two', 0), 100, 4);
        } elseif ($type == 2) {
            ...
            $channelIdentityServices = app()->make(ChannelMerchantIdentityServices::class);
            $channel_level_list = $channelIdentityServices->getList(['is_show' => 1, 'is_del' => 0])['list'];
            foreach ($channel_level_list as &$item) {
                $item['discount'] = bcmul($item['discount'], '0.01', 2);
            }
            $data['channel_level_list'] = $channel_level_list;
        }
...
//        });
        foreach ($attrValue as &$val) {
            if ($type == 2) {
                ...
                $val['channel_level_price'] = $val['channel_level_price'] ? json_decode($val['channel_level_price'], true) : [];
                if ($data['storeInfo']['channel_type'] == 1) {
                    continue;
                }
                // 获取已存在的等级 id 数组
                $channelExistingIds = array_column($val['channel_level_price'], 'id');

                // 补全缺失的等级值
                foreach ($data['channel_level_list'] as $channelLevel) {
                    if (!in_array($channelLevel['id'], $channelExistingIds)) {
                        // 计算缺失的 price 值
                        $channelPrice = bcmul($val['price'], $channelLevel['discount'], 2);
                        $val['channel_level_price'][] = ['id' => $channelLevel['id'], 'price' => $channelPrice];
                    }
                }
            }
        }
    ...
    public function otherUpdate(int $id, int $type, array $data)
    {
        ...
        if ($type == 1) {
            ...
            $proData['channel_type'] = $data['channel_type'];
        }
        ...
        foreach ($data['attr_value'] as $item) {
            ...
            if ($type == 1) {
                ...
                $attrData[$item['id']]['channel_level_price'] = isset($item['channel_level_price']) && $item['channel_level_price'] ? json_encode($item['channel_level_price']) : '';
            }
        }
     ...
     */
    public function productImport($file, $supplierId = 0)
    {
        $all = $success = $fail = 0;
        ...
        foreach ($importData as $sku) {
            ...
            if (!isset($productData[$sku['id']])) {
                ...
                $productData[$sku['id']]['is_brokerage'] = 0;
                $productData[$sku['id']]['share_content'] = '';
                $productData[$sku['id']]['min_qty'] = $sku['min_qty'];
                $productData[$sku['id']]['presale_status'] = 0;
                $productData[$sku['id']]['product_clear'] = ['is_general_product'];
                if ($supplierId != 0) {
                    $productData[$sku['id']]['type'] = 2;
                    $productData[$sku['id']]['relation_id'] = $supplierId;
                }
            }
            $detail = [];
    ...
    {
        $store_brokerage_status = (int)sys_config('store_brokerage_status', 0);
        $store_brokerage_ratio = (float)bcdiv(sys_config('store_brokerage_ratio', 0), 100, 4);
        $store_brokerage_two = (float)bcdiv(sys_config('store_brokerage_two', 0), 100, 4);

        $member_func_status = (int)sys_config('member_func_status', 0);
        /** @var SystemUserLevelServices $systemUserLevelServices */
        $systemUserLevelServices = app()->make(SystemUserLevelServices::class);
        $level_list = $systemUserLevelServices->getWhereLevelList([], 'id,name,grade,discount');
        foreach ($level_list as &$level_item) {
            $level_item['discount'] = bcmul($level_item['discount'], '0.01', 2);
        }

        $channel_func_status = (int)sys_config('channel_func_status', 0);
        $channelIdentityServices = app()->make(ChannelMerchantIdentityServices::class);
        $channel_level_list = $channelIdentityServices->getList(['is_show' => 1, 'is_del' => 0])['list'];
        foreach ($channel_level_list as &$channel_level_item) {
            $channel_level_item['discount'] = bcmul($channel_level_item['discount'], '0.01', 2);
        }
        return compact('store_brokerage_status', 'member_func_status', 'channel_func_status', 'level_list', 'channel_level_list', 'store_brokerage_ratio', 'store_brokerage_two');
    }
}
