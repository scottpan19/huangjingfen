...
//        $countWhere = $store_id != -1 ? ['pid' => 0, 'store_id' => $store_id] : ['pid' => 0];
        ...
        $data['yue_pay_status'] = (int)sys_config('balance_func_status') ? (int)1 : (int)2;//余额支付 1 开启 2 关闭
        ...
        if ($order['is_send_gift'] == 1) {
            if ($order['receive_gift_uid'] != 0) {
                ...
                $order['receive_gift_user_info'] = [
                    ...
                    'receive_gift_nickname' => $receiveGiftUser['nickname'] ?? '用户已注销',
                    'receive_gift_avatar' => $receiveGiftUser['avatar'] ?? sys_config('h5_avatar'),
                ];
            }
    ...
    public function tidyOrderList(array $data, bool $is_cart_info = true, bool $abridge = false)
    {
        ...
        foreach ($data as &$item) {
            ...
            if ($item['paid'] == 1) {
                ...
                $item['pay_type_name'] = $item['pay_type'] != 'yue' && $item['yue_price'] > 0 ? $item['pay_type_name'] . '（余额支付）' : $item['pay_type_name'];
            } else {
...
//        }
     ...
     * 修改订单商品价格
     * @param int $id 订单ID
     * @param array $data 商品价格数据
     * @param int $is_postage 是否免运费
     * @return bool|string
     */
...
//        $data['offline_pay_status'] = (int)sys_config('offline_pay_status') ?? (int)2;
        $data['yue_pay_status'] = (int)sys_config('balance_func_status') ? (int)1 : (int)0;//余额支付 1 开启 2 关闭
//        $data['pay_weixin_open'] = (int)sys_config('pay_weixin_open') ?? 0;//微信支付 1 开启 0 关闭
//        $data['ali_pay_status'] = (bool)sys_config('ali_pay_status');//支付包支付 1 开启 0 关闭
    ...
    public function checkPaytype(string $payType)
    {
        ...
        switch ($payType) {
            ...
            case PayServices::YUE_PAY:
                $res = sys_config('balance_func_status');
                break;
            case 'offline':
    ...
    {
        $res = $this->dao->update($orderInfo['id'], ['pay_type' => 'offline']);
        if ($res) {
            //支付成功后向管理员发送模板消息
        ...
        if ($cart_ids) {
            ...
            foreach ($cartInfo as &$item) {
                if (isset($item['postage_price']) && $item['postage_price']) {
                    $postage_price = bcdiv($item['postage_price'], $item['cart_num'], 4);
                    if ($postage_price > 0) {
                        $item['truePrice'] = bcadd($item['truePrice'], $postage_price, 2);
                    }
                }
//                    bcmul((string)$item['truePrice'], (string)$rate, 2);
    ...
    public function editAddress($id, $data)
    {
        ...
        OrderStatusJob::dispatch([$id, 'edit_address', ['change_message' => '修改地址', 'change_manager_id' => request()->adminId(), 'change_manager_type' => 'admin']]);
        return true;
    }
}
