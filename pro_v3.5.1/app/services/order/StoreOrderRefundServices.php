...
//            0 => ['name' => '全部', 'num' => $this->dao->count($del_where)],
    ...
    public function applyRefund(int $id, int $uid, $order = [], array $cart_ids = [], int $apply_type = 1, float $apply_price = 0.00, array $refundData = [], $admin = 0)
    {
        ...
        $refundData['refund_type'] = $apply_type;
        ...
        $refundId = $this->transaction(function () use ($id, $order, $cart_ids, $refundData, $storeOrderCartInfoServices, $cartInfo, $cartInfos, $admin, $apply_price) {
            ...
            OrderStatusJob::dispatch([$order['id'], 'apply_refund', ['change_message' => $change_message, 'change_manager_id' => request()->adminId(), 'change_manager_type' => $change_manager_type]]);
            $res1 = true;
            $res2 = true;
            //添加退款数据
...
//                $res2 = false !== $this->storeOrderServices->update(['id' => $order['id']], ['refund_status' => 1]);
    ...
    public function refuseRefund(int $id, array $data, $orderRefundInfo = [])
    {
        ...
        $this->transaction(function () use ($id, $orderRefundInfo, $data) {
            ...
            //记录
            OrderStatusJob::dispatch([$oid, 'refund_n', ['change_message' => '拒绝退款原因:' . ($data['refund_reason'] ?? $data['refuse_reason'] ?? ''), 'change_manager_id' => request()->adminId(), 'change_manager_type' => 'admin']]);
        });
    ...
    public function agreeRefund(int $id, array $refundData, string $admin = '')
    {
        $order = $this->transaction(function () use ($id, $refundData, $admin) {
            ...
            $refunded_price = $refundOrder['refunded_price'];
            //退金额
            ...
            if ($refundData['refund_price'] > 0) {
                ...
                $refundData['pay_price'] = $refundOrder['online_price'];
                //余额支付金额
                $yue_price = $order['yue_price'];
                //线上支付金额
                $online_price = $order['online_price'];
                //已退款金额
                //申请退款金额$refundData['refund_price']

                // 计算线上退款剩余金额
                $online_refund_remaining = bcsub((string)$online_price, (string)$refunded_price, 2);
                $online_refund_remaining = max($online_refund_remaining, 0);

                // 先处理线上退款部分
                if ($online_refund_remaining > 0) {
                    // 计算线上退款金额
                    $online_refund_amount = min($refundData['refund_price'], $online_refund_remaining);

                    // 如果线上退款不够，剩余部分用余额退款
                    if ($refundData['refund_price'] > $online_refund_amount) {
                        $yue_refund_amount = bcsub((string)$refundData['refund_price'], (string)$online_refund_amount, 2);
                    }
                } else {
                    // 线上已全部退完，全部走余额退款
                    $yue_refund_amount = $refundData['refund_price'];
                }
                // 先处理线上退款
                if ($online_refund_amount > 0) {
                    $refundData['refund_price'] = $online_refund_amount;
...
//                            break;
                ...
                // 处理余额退款部分
                if ($yue_refund_amount > 0) {
                    $yueRefundData = $refundData;
                    $yueRefundData['refund_price'] = $yue_refund_amount;

                    //余额退款
                    if (!$this->yueRefund($refundOrder, $yueRefundData)) {
                        throw new ValidateException('余额退款失败');
                    }
                }
            }
            ...
            OrderStatusJob::dispatch([$order['id'], 'refund_price', ['change_message' => '退款给用户：' . bcadd($online_refund_amount, $yue_refund_amount, 2) . '元; ' . $admin, 'change_manager_id' => request()->adminId(), 'change_manager_type' => 'admin']]);
            return $order;
        });
        //订单同意退款事件
    ...
    public function yueRefund($order, array $refundData)
    {
        ...
        return $res && $userMoneyServices->income('pay_product_refund', $order['uid'], $refundData['refund_price'], bcadd((string)$usermoney, (string)$refundData['refund_price'], 2), $order['id'], '订单退款返还余额');
    }
    ...
    public function regressionIntegral($order)
    {
        ...
        if ($give_integral) {
            ...
            if (!$count) {
                ...
                $userBillServices->update(['type' => 'gain', 'link_id' => $order['id']], ['frozen_time' => 0]);
                //记录赠送积分收回
     ...
     * @return bool
     */
    public function regressionStock($order, int $is_stock = 1, string $refund_order_id = '')
    {
        ...
        foreach ($cartInfo as $cart) {
            ...
            $product_type = $cart['cart_info']['productInfo']['product_type'] ?? '';
            switch ($order['type']) {
                case 0://普通
                case 6://预售
                case 8://抽奖
                    if ($product_type === 0 && $is_stock == 1) {
                        $inInventory[] = [
                            'product_id' => (int)$cart['cart_info']['productInfo']['id'],
                            'unique' => $unique,
                            'inventory' => $cart_num
...
//        return false;
        if (count($inInventory) > 0) {
            $order_id = $refund_order_id ?: $order['order_id'];

            event('product.stock.record', [
                $refund_order_id ? StockRecord::STOCK_TYPE_RETURN : StockRecord::STOCK_TYPE_OTHER_IN,
                StockRecord::TYPE_IN,
                $inInventory,
                getAdminId(),
                $order_id
    ...
    public function refundDetail($uni, array $field = ['*'], array $with = ['invoice', 'virtual'])
    {
        ...
        foreach ($orderData['cartInfo'] ?? [] as $key => &$cart) {
            ...
            $promotionsPrice = bcadd($promotionsPrice, (string)($cart['sum_promotions_price'] ?? 0), 2);
        }
        //优惠活动优惠详情
