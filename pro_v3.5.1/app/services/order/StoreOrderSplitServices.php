...
class StoreOrderSplitServices extends BaseServices
{
    ...
    public function equalSplit(int $id, array $cart_ids, $orderInfo = [], int $store_id = 0, bool $is_refund = false, int $erp_id = 0)
    {
        ...
        $order = $this->transaction(function () use ($id, $data, $orderInfo, $store_id, $storeOrderCartInfoServices, $split_cart_ids, $is_refund, $change_price, $order_pay_price, $erp_id) {
            $order = [];
            $i = 0;
            $yue_price = $orderInfo['yue_price'];
            $online_price = $orderInfo['online_price'];
            foreach ($data as $key => $cart_ids) {
                ...
                //处理订单
                [$online_price, $yue_price] = $this->splitComputeOrder((int)$new_id, $cart_data, $orderInfo, (float)($change_price ? $order_pay_price : 0), (float)$orderInfo['pay_price'], (float)($order[0]['pay_price'] ?? 0), $online_price, $yue_price);
...
//        return $split_order_info;
     ...
     * @return bool|float[]
     */
    public function splitComputeOrder(int $id, array $cart_info_data, $orderInfo = [], float $order_pay_price = 0.00, float $pay_price = 0.00, float $pre_pay_price = 0.00, float $online_price = 0.00, float $yue_price = 0.00)
    {
        ...
        foreach ($cart_info_data as $cart) {
            ...
            $promotions_price = bcadd((string)$promotions_price, (string)($_info['sum_promotions_price'] ?? '0'), 2);
        }
        ...
        if($online_price >= $order_update['pay_price']){
            // online_price足够支付全部金额
            $order_update['online_price'] = $order_update['pay_price'];
            $order_update['yue_price'] = 0;
            $online_price = bcsub((string)$online_price, (string)$order_update['pay_price'], 2); // 剩余online_price
            // yue_price不变，全部剩余
        }else{
            // online_price不够，全部用完，剩余部分用yue_price
            $order_update['online_price'] = $online_price;
            $remaining_pay = bcsub((string)$order_update['pay_price'], (string)$online_price, 2); // 剩余需要支付的金额

            if($yue_price >= $remaining_pay){
                // yue_price足够支付剩余金额
                $order_update['yue_price'] = $remaining_pay;
                $yue_price = bcsub((string)$yue_price, (string)$remaining_pay, 2); // 剩余yue_price
            }else{
                // yue_price也不够，全部用完
                $order_update['yue_price'] = $yue_price;
                $yue_price = 0; // yue_price用完
            }

            $online_price = 0; // online_price已用完
        }
