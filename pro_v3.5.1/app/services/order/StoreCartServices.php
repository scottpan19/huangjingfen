...
class StoreCartServices extends BaseServices
{
    ...
    public function changeUserCartNum($id, $number, $uid)
    {
        ...
        if ($carInfo->cart_num == $number) return true;
        //购物车修改数量检查限购
        $limitInfo = $StoreProduct->get($carInfo->product_id, ['min_qty']);
        if ($number < $limitInfo['min_qty']) {
            throw new ValidateException('不能小于起购数量');
        }
        ...
        foreach ($cartList as &$item) {
            ...
            if (isset($productInfo['attrInfo']['product_id']) && $item['product_attr_unique']) {
                ...
                if ($item['is_channel'] == 1) {
                    [$truePrice, $channelPrice] = $productServices->setChannelPrice($productInfo['attrInfo']['price'] ?? 0, $uid, ['channel_type' => $productInfo['channel_type'] ?? 1, 'channel_level_price' => $productInfo['attrInfo']['channel_level_price'] ?? '']);
                    $item['truePrice'] = $truePrice;
