...
class SystemConfigServices extends BaseServices implements ServeConfigInterface
{
     ...
     * @param bool|array $control 控制组件
     * @param array $controle_two 二级控制组件
     * @param array $controlle_three 三级控制组件
     * @return array
     */
...
//            ])
    ...
    public function shopPayFormBuild()
    {
        ...
        $data = $this->getConfigAllField([
            ...
            'balance_func_status', 'offline_pay_status',
    ...
    public function saveUserConfig(string $type, array $data)
    {
        switch ($type) {
            case 'basic'://基础
                break;
            case 'register'://注册
                if ($data['newcomer_limit_status'] == 1) {
                    if ($data['newcomer_limit_time'] < 1) {
                        throw new AdminException('请填写新人注册活动的时间');
                    }
                }
...
//            ]),
    ...
    public function channelFormBuild()
    {
        ...
        $data = $this->getConfigAllField([
            'system_channel_style', 'channel_apply_phone_verify', 'channel_apply_explain', 'channel_func_status'
        ]);
        $build->rule([
            Build::tabs()->option('采购商设置', [
                Build::radio('channel_func_status', $data['channel_func_status']['info'], (int)$data['channel_func_status']['value'])->info($data['channel_func_status']['desc'])->options($this->getOptions($data['channel_func_status']['parameter'])),
