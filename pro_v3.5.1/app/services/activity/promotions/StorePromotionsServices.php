        ...
        if ($list) {
            ...
            $getPromotions = function ($cartList, $oids) {
                ...
                foreach ($cartList as $key => $cart) {
                    ...
                    $promotionsPrice = bcadd((string)$promotionsPrice, (string)($info['sum_promotions_price'] ?? 0), 2);
                        ...
                        if ($sumCount >= $p['threshold']) {//满足
                            ...
                            if ($sum_promotions_price) {
                                ...
                                foreach ($useProductArr as $value) {
                                    ...
                                    $price = $value['price'];
                                    if ($count > 1) {
                                        // 提高分摊比例精度从4位到8位，减少精度损失
                                        $promotions_true_price = (float)bcmul((string)bcdiv((string)$price, (string)$useSumPrice, 8), (string)$sum_promotions_price, 2);
                                        ...
                                        // 最后一件商品使用补偿机制，确保总优惠金额精确
                                        $one_promotions_sum_price = (float)bcsub((string)$sum_promotions_price, (string)$compute_price, 2);
                                        $promotions_true_price = (float)bcdiv((string)$one_promotions_sum_price, (string)$value['cart_num'], 4);
                                    }
                                    $data[$unique] = ['product_id' => $product_id, 'unique' => $unique, 'price' => $price, 'sum_promotions_price' => $one_promotions_sum_price, 'promotions_true_price' => $promotions_true_price];
                                    $count--;
                                }
                        ...
                        if ($valid) {
                            ...
                            foreach ($promotionsProductArr as $product) {
                                ...
                                if ($valid['discount_type'] == 1) {
                                    if ($productCount > 1) {
                                        // 提高分摊比例精度从4位到8位，减少精度损失
                                        $promotions_true_price = (float)bcmul((string)bcdiv((string)$price, (string)$sumPrice, 8), (string)$sum_promotions_price, 2);
                                        ...
                                        // 最后一件商品使用补偿机制，确保总优惠金额精确
                                        $one_promotions_sum_price = (float)bcsub((string)$sum_promotions_price, (string)$compute_price, 2);
                                        $promotions_true_price = (float)bcdiv((string)$one_promotions_sum_price, (string)$product['cart_num'], 4);
                                    }
                                } else {
                                    $promotions_true_price = (float)bcmul((string)$price, (string)$promotionsDiscount, 2);
                                    if ($promotions_true_price < 0.01) {//实际优惠小于0.01 就不计算优惠
