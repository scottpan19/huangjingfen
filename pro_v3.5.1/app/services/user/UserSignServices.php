...
//        $signList = array_chunk($signList, 7);
    ...
    public function signDataFilling($signMode, $uid, $sign_give_point, $sign_exp, $dateList, $time)
    {
        ...
        //获取累积签到和连续签到
        $cumulativeSignDays = $this->dao->getCumulativeDays($uid, $sign_mode); //获取累积签到天数
    ...
    public function sign(int $uid)
    {
        ...
        //签到是否关闭
        if (!sys_config('sign_status', 1)) {
            throw new ApiException('签到暂未开放');
        }
        //检测今天是否已经签到
        ...
        //累积签到天数
        $cumulativeDays = $this->dao->getCumulativeDays($uid, $sign_mode) + 1;
