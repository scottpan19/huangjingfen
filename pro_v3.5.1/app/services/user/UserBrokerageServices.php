...
//            $item['sum_number'] = $item['income'] > $item['pay'] ? bcsub($item['income'], $item['pay'], 2) : 0;
    ...
    public function orderRefundBrokerageBack($order)
    {
        $id = (int)$order['id'];
        $where = [
            'uid' => [$order['spread_uid'], $order['spread_two_uid'], $order['division_staff_id'], $order['division_agent_id'], $order['division_id']],
    ...
    public function userBrokerageList(int $uid, $data = [])
    {
        ...
        if ($list) {
            ...
            foreach ($list as &$item) {
                ...
                $item['frozen_time'] = $item['frozen_time'] ? date('Y/m/d H:i', (int)$item['frozen_time']) : '';
            }
