...
class UserExtractServices extends BaseServices
{
    ...
    /**
     * 处理微信提现
     * @param array $userExtract 提现记录
     * @param float $extractNumber 提现金额
     * @return bool
     */
    protected function processWechatExtract($userExtract, $extractNumber)
    {
        if (sys_config('brokerage_type', 0)) {
            ...
            $res = Payment::merchantPay($userExtract['channel_type'], $userExtract['uid'], $wechat_order_id, (string)$extractNumber, '提现佣金到零钱', $userExtract['user_name'] ?? '用户', $transferDetailList);
            if (!$res) {
    ...
    public function cancel($id, $uid)
    {
        ...
        if ($extract['status'] != 0) {
            throw new ValidateException('提现状态不是待审核，不能取消');
        }
        if ($extract['uid'] != $uid) {
            throw new ValidateException('提现不属于当前用户');
        }
        $res = $this->changeFail($id, $extract, '用户取消提现申请');
        if ($res) {
            return true;
        } else {
