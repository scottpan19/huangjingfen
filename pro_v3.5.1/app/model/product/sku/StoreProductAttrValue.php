...
class StoreProductAttrValue extends BaseModel
{
    ...
    {
        if ($value !== '') {
            $query->whereLike('product_id|product_name|product_code|product_bar_code', '%' . trim($value) . '%');
        }
    }
    ...
    public function searchTimeAttr($query, $value)
    {
        if ($value !== '') {
            ...
            $query->whereBetweenTime('add_time', $time[0], $time[1]);
        }
    ...
    public function searchStockRangeAttr($query, $value)
    {
        $stock_range = explode('-', $value);
        if (count($stock_range) == 0) {
            return;
        }
