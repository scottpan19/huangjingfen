...
class UserBatchJob extends BaseJobs
{
     ...
     * @return bool
     */
    public function userBatch($type, $uids, $data, $label_type = 0)
    {
        if (!$type || !$uids || !$data) {
            return true;
        }
        ...
        foreach ($uidsArr as $ids) {
            //加入分批队列
            self::dispatchDo('chunkUserBatch', [$type, $ids, $data, $label_type]);
        }
        return true;
    }
     ...
     * @return bool
     */
    public function chunkUserBatch($type, $uids, $data, $label_type = 0)
    {
        if (!$type || !$uids || !$data) {
            return true;
        }
        foreach ($uids as $id) {
            self::dispatchDo('runUserBatch', [$type, $id, $data, $label_type]);
        }
        return true;
    }
     ...
     * @return bool
     */
    public function runUserBatch($type, $uid, $data, $label_type = 0)
    {
        $uid = (int)$uid;
        if (!$type || !$uid || !$data) {
            return true;
            ...
            switch ($type) {
                ...
                case 2://标签
                    ...
                    if ($label_id) {
                        ...
                        $services->setUserLable($uid, $label_id, 0, 0, false, $label_type);
                    }
                    break;
                case 3://等级
