...
function getMenusName() {
  let storage = window.localStorage,
    menuList,
    menuData;
  menuList = isSupplierPath()
    ? storage.getItem("supplier_menuList")
    : storage.getItem("menuList");
  try {
...
export default {
  ...
  mutations: {
    getmenusNav(state, menuList) {
      ...
      let storage = window.localStorage;
      if (isSupplierPath()) {
        storage.setItem("supplier_menuList", JSON.stringify(menuList));
        storage.setItem("supplier_roterPre", "supplier");
      } else {
