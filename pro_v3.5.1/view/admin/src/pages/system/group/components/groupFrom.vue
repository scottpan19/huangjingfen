        ...
        <Row type="flex" :gutter="24" class="maxH">
          ...
          <Col span="24">
            <FormItem label="数据字段：" prop="config_name">
              <Input
                v-model="formValidate.config_name"
                placeholder="请输入数据字段"
                class="input-add"
              ></Input>
            </FormItem>
          ...
          <Col span="24">
            <div
              class="flex"
              v-for="(item, index) in formValidate.typelist"
              :key="index"
            >
              <div class="mr10">
                <FormItem
              ...
              <Col
                span="24"
                v-if="
                  item.type.value === 'radio' ||
                  item.type.value === 'checkbox' ||
                  item.type.value === 'select'
                "
              >
                <FormItem
  ...
  methods: {
    ...
    handleSubmit(name) {
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          if (this.formValidate.typelist.length === 0)
            return this.$Message.error("请添加字段名称：姓名！");
          groupAddApi(data)
            .then(async (res) => {
          ...
          if (!this.formValidate.name)
            return this.$Message.error("请添加数据组名称！");
          if (!this.formValidate.config_name)
            return this.$Message.error("请添加数据字段！");
          if (!this.formValidate.info)
            return this.$Message.error("请添加数据简介！");
        }
      });
    },
    handleReset() {
