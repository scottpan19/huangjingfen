<template>
  <div>
    <Drawer :closable="false" width="1000" class-name="order_box" v-model="modals" :styles="{ padding: 0 }">
      <div v-if="orderDatalist">
        <div class="head">
          <div class="full">
            ...
            <div v-if="rowActive.delete_time == null">
              ...
              " @click="changeMenu('1')">确认收款
              </Button>
              <Button v-if="orderData._status_new == 1" @click="edit">订单改价</Button>
              <Button :class="openErp ? 'on' : ''" :disabled="openErp" @click="sendOrder" v-if="
                ...
                !orderData.refund.length && orderData.supplier_id === 0 && !isSupplier
              ">发送货</Button>
              <Button :class="openErp ? 'on' : ''" :disabled="openErp" @click="sendOrder" v-if="
                (orderData._status_new === 2 ||
                  orderData._status_new === 8 ||
                ...
                (orderData.pinkStatus === null ||
                  orderData.pinkStatus === 2) &&
                !orderData.refund.length && isSupplier
              ">发送货</Button>
              ...
              <Button :class="openErp ? 'on' : ''" :disabled="openErp" v-if="orderData.kuaidi_label"
                @click="changeMenu('13')">快递面单打印
              </Button>
              <!-- apply_type 申请类型
                  refund_type 退款处理类型
                  pay_price 退款金额
              ...
              <Button :class="openErp ? 'on' : ''" :disabled="openErp" v-if="
                ...
                !formType
              " @click="changeMenu('5')">立即退款
              </Button>
              <Button :class="openErp ? 'on' : ''" :disabled="openErp" v-if="
                [2, 3].includes(orderData.apply_type) &&
                [0, 1, 2].includes(orderData.refund_type) &&
                !formType
              " @click="changeMenu('55')">同意退货
              </Button>
              <Button :class="openErp ? 'on' : ''" :disabled="openErp"
                v-if="[0, 1, 2, 5].includes(orderData.refund_type) && !formType" @click="changeMenu('7')">不退款
              </Button>
              ...
              <Dropdown @on-click="changeMenu" v-if="orderData._status_new !== 1 && formType">
                ...
                <DropdownMenu slot="list">
                  ...
                  <DropdownItem :disabled="openErp" v-if="
                    [0, 1, 4, 5].includes(orderData.refund_type) &&
                    orderData.paid == 1 &&
                    orderData.refund_status !== 2 &&
                    parseFloat(orderData.pay_price) > 0 && !orderData.refund.length
                  " name="5">立即退款
                  </DropdownItem>
                  <!--  &&
                  ...
                  <DropdownItem v-if="orderData._status_new >= 2" name="10">小票打印</DropdownItem>
          ...
          <ul class="list">
            ...
            <li class="item" v-if="orderDatalist.orderInfo.online_price > 0">
              <div class="title">现金支付</div>
              <div>
                ¥{{ orderDatalist.orderInfo.online_price }}
              </div>
            </li>
            <li class="item" v-if="orderDatalist.orderInfo.yue_price > 0">
              <div class="title">余额抵扣</div>
              <div>¥{{ orderDatalist.orderInfo.yue_price }}</div>
            </li>
        ...
        <Tabs v-model="activeName">
          <TabPane label="订单信息" name="detail">
            <div class="section" v-if="!formType">
              ...
              <ul class="list">
                ...
                <li class="item">
                  <div>退款凭证：</div>
                  <div class="value" v-if="orderDatalist.orderInfo.refund_img.length > 0">
            ...
            <div class="section">
              ...
              <ul class="list">
                ...
                <li class="item" v-if="orderDatalist.orderInfo.yue_price > 0">
                  <div>余额抵扣：</div>
                  <div class="value">
                    ￥{{ orderDatalist.orderInfo.yue_price || 0.0 }}
                  </div>
                </li>
                <li class="item" v-if="orderDatalist.orderInfo.online_price > 0">
                  <div>现金支付：</div>
                  <div class="value">
                    ￥{{ orderDatalist.orderInfo.online_price || 0.0 }}
                  </div>
                </li>
                <li class="item" v-if="Number(orderDatalist.orderInfo.channel_price) > 0">
                  <div>采购优惠：</div>
                  <div class="value">
                    ￥{{ orderDatalist.orderInfo.channel_price }}
                  </div>
                </li>
                <li class="item">
                  <div>支付时间：</div>
            ...
            <div v-if="orderDatalist.orderInfo.custom_form.length && isShow" class="section">
              ...
              <ul class="list">
                <li v-for="(item, index) in orderDatalist.orderInfo.custom_form" :key="index" class="item" v-show="(item.value &&
            ...
            <div v-if="orderDatalist.orderInfo.invoice" class="section">
              ...
              <ul class="list">
                ...
                <li v-if="orderDatalist.orderInfo.invoice.type == 2" class="item">
                  <div>开户银行：</div>
                  <div class="value">
                ...
                <Tooltip theme="dark" max-width="300" :delay="600">
                  <div class="product">
                    <div class="image" v-viewer>
                      <img v-lazy="row.productInfo.attrInfo
          ...
          <TabPane label="发货记录" name="recordList" v-if="splitList.length" :index="1">
            <Table :columns="columnSplit" :data="splitList" :loading="loading" no-data-text="暂无数据" highlight-row
              ...
              <template slot-scope="{ row }" slot="product">
                <Tooltip theme="dark" max-width="300" :delay="600">
                  <div class="product productTime" v-for="(j, index) in row._info" :key="index">
                    <div class="image" v-viewer>
                      <img v-lazy="j.cart_info.productInfo.attrInfo
              ...
              <template slot-scope="{ row, index }" slot="action">
                ...
                <a :disabled="openErp" @click="sendOrder(row, 1)" v-if="
                  ...
                  row.store_id == 0 && row.supplier_id === 0 && !isSupplier
                ">发送货</a>
                <a :disabled="openErp" @click="sendOrder(row, 1)" v-if="
                  (row._status === 2 ||
                    row._status === 8 ||
                  ...
                  row.shipping_type === 1 &&
                  (row.pinkStatus === null || row.pinkStatus === 2) &&
                  row.store_id == 0 && isSupplier
                ">发送货</a>
                <a :disabled="openErp" @click="delivery(row, 1)" v-if="row._status === 4 && !row.split.length">配送信息</a>
                ...
                <a :disabled="openErp" @click="btnClick(row)" v-if="
          ...
          "
                ...
                <template>
                  <Dropdown :transfer="true" @on-click="changeMenu('', row, $event, 1)">
                    ...
                    <DropdownMenu slot="list">
                      <DropdownItem :disabled="openErp" name="1" ref="ones" v-show="row._status === 1 &&
                        row.paid === 0 &&
                        row.pay_type === 'offline'
                        ">立即支付</DropdownItem>
                      <!-- <DropdownItem name="2">订单详情</DropdownItem> -->
                      <DropdownItem :disabled="openErp" name="3">订单记录</DropdownItem>
                      <DropdownItem :disabled="openErp" name="11" v-show="row._status >= 3 && row.express_dump">电子面单打印
                      </DropdownItem>
                      <DropdownItem name="10" v-show="row._status >= 2">小票打印</DropdownItem>
                      <DropdownItem name="13" v-if="row._status >= 3 && row.kuaidi_label">快递面单打印</DropdownItem>
                      <DropdownItem name="4" v-show="row._status !== 1 ||
                        (row._status === 3 &&
                          row.use_integral > 0 &&
                          row.use_integral >= row.back_integral)
                        ">订单备注</DropdownItem>
                      <DropdownItem :disabled="openErp" name="5" v-show="row.refund_type != 2 &&
                        row.refund_type != 4 &&
                        row.refund_type != 6 &&
                        row.paid == 1 &&
                        row.refund_status !== 2 &&
                        ...
                        ">立即退款</DropdownItem>
                      <DropdownItem :disabled="openErp" name="55" v-show="row.refund_type == 2">同意退货</DropdownItem>
                      <DropdownItem :disabled="openErp" name="8" v-show="row._status === 4 && !row.split.length">已收货
                      </DropdownItem>
                      <DropdownItem :disabled="openErp" name="9" v-if="row.is_del == 1">删除订单</DropdownItem>
                    </DropdownMenu>
                  </Dropdown>
                </template>
              </template>
          ...
          <TabPane label="核销记录" name="writeOff" v-if="offRecordList.length" :index="2">
            <Table :columns="columns3" :data="offRecordList" :loading="loading" no-data-text="暂无数据" highlight-row
              no-filtered-data-text="暂无筛选结果">
              <template slot-scope="{ row }" slot="product">
                <div class="product">
                  <div class="image" v-viewer>
    ...
    <Modal v-model="showAddressModal" scrollable title="修改地址" footer-hide width="540">
      <div>
        <Form ref="formEditAddress" :model="formEditAddress" :label-width="80">
          <FormItem label="用户名称：">
        ...
        <div class="acea-row row-right" slot="footer">
          <Button @click="showAddressModal = false">取消</Button>
          <Button type="primary" class="ml-14" @click="saveAddressEdit">保存</Button>
        </div>
      </div>
    </Modal>
  </div>
...
export default {
  ...
  watch: {
    orderDatalist(value) {
      ...
      this.formEditAddress.user_address = value.orderInfo.user_address;
      this.formEditAddress.user_phone = value.orderInfo.user_phone;
      this.formEditAddress.real_name = value.orderInfo.real_name;
      this.getList(
  ...
  methods: {
    ...
    getSplitOrder(id) {
      ...
      splitOrder(id, { status: 2 })
        .then((res) => {
          ...
          this.$store.commit("admin/order/setSplitOrderData", res.data);
        })
        .catch((err) => {
    ...
    changeMenu(value, row, name, num) {
      if (num) {
        ...
        if (value == 5) {
          let rowData = this.splitList.length
            ? this.splitList[0]
            : this.rowActive;
    ...
    saveAddressEdit() {
      orderEditAddressApi(this.orderId, this.formEditAddress)
        .then((res) => {
          this.showAddressModal = false;
          this.$Message.success(res.msg);
          this.orderData.user_address = this.formEditAddress.user_address;
          this.orderData.user_phone = this.formEditAddress.user_phone;
          this.orderData.real_name = this.formEditAddress.real_name;
        })
        .catch((res) => {
          this.$Message.error(res.msg);
        });
