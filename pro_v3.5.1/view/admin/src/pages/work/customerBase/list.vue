  ...
  <div>
        ...
        <Form :label-width="96" inline :label-colon="true">
          ...
          <FormItem label="客户标签">
            <div class="check_label acea-row row-middle" @click="setLabel(0)">
              <Tag
                v-for="(item, index) in labelFormList"
                :key="index"
                :name="item.title"
                closable
    ...
    <Card :bordered="false" dis-hover class="ivu-mt">
      ...
      >
        ...
        <template slot-scope="{ row }" slot="external_userid">
          <span v-if="row.followOne">{{
            row.followOne.member.mastare_department_name
          }}</span>
        </template>
        <!--  -->
    ...
    >
      <div class="label-wrapper">
        ...
        <div class="footer">
          <Button type="primary" ghost @click="labelCancel()">取消</Button>
          <Button type="primary" class="ml-14" @click="labelConfirm()"
            >确定</Button
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    editMaekConfirm() {
      workClientEdit(this.selectRow.id, { remark: this.remark })
        .then((res) => {
          this.$Message.success(res.msg);
          this.getList();
        })
