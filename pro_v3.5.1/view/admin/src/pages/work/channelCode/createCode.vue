...
export default {
  ...
  methods: {
    ...
    getWorkLabel() {
      workLabel().then((res) => {
        // 将 label_id 统一转为字符串进行匹配，避免类型不一致
        const ids = new Set((this.formItem?.label_id || []).map(String));
        this.dataLabel = this.collectLabels(res.data, ids);
    ...
    mapTree(org) {
      const ids = new Set((this.formItem?.label_id || []).map(String));
      this.dataLabel = this.collectLabels(org, ids);
    },
    // 单次遍历收集匹配的标签，避免 O(n*m) 双循环
    collectLabels(tree, ids) {
      if (!Array.isArray(tree) || !(ids instanceof Set) || ids.size === 0) {
        return [];
      }
      const normalizedIds = new Set([...ids].map((v) => String(v)));
      const result = [];
      const seen = new Set();
      for (const group of tree) {
        ...
        for (const child of children) {
          ...
          if (key && normalizedIds.has(key) && !seen.has(key)) {
            result.push({
