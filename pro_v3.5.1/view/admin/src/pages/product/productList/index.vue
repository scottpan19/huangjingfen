      ...
      <div class="acea-row row-between-wrapper">
        <div class="Button">
          <Button
            ...
            >添加商品</Button
          >
          <router-link to="/supplier/product/add_product?productType=0" v-else>
      ...
      >
        ...
        >
          <template #default="{ row, rowIndex }">
            ...
            <Divider type="vertical" />
            <a @click="stockControl(row)" v-if="row.product_type != 1">库存</a>
            ...
            <template>
              ...
              >
                ...
                <DropdownMenu slot="list">
                  ...
                  <DropdownItem
                    name="11"
                    v-if="!isSupplier && row.is_channel_product"
                    >采购价</DropdownItem
                  >
                  ...
                  <DropdownItem
                    name="4"
                    v-if="['1', '4', '5'].includes(artFrom.type)"
                    >商品预览</DropdownItem
                  >
                  <DropdownItem name="5" v-if="['1', '2'].includes(artFrom.type)">强制下架</DropdownItem>
...
import merPriceSet from "../components/merPriceSet.vue";
...
export default {
  ...
  methods: {
    ...
    // 复制淘宝
    onCopy() {
      let path = this.isSupplier ? '/supplier/product/add_product' : '/admin/product/add_product'
      this.$router.push({
        path: path,
        query: { type: -1 },
      });
    ...
    pageChange(currentPage) {
      ...
      this.tableForm.page = currentPage;
      this.getDataList();
    },
    // 表格搜索
    userSearchs(e) {
    ...
    edit(row) {
      let path = "";
      path = !this.__isSupplierPath()
        ? `/admin/product/add_product?id=${row.id}&from_page=${this.artFrom.page}`
        : `/supplier/product/add_product?id=${row.id}&from_page=${this.artFrom.page}`;
      this.$router.push({
        path: path,
      });
    },
