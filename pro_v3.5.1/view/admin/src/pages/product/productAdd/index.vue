    ...
    <div class="i-layout-page-header">
      <PageHeader class="product_tabs" hidden-breadcrumb>
        <div slot="title" class="acea-row row-middle">
          <router-link
            :to="{
              name: !isSupplier ? 'product_productList' : 'supplier_product_productList',
    ...
    <Card :bordered="false" dis-hover class="ivu-mt">
      ...
      >
        ...
        <div v-show="currentTab === '2'">
                  ...
                  <template slot-scope="scope">
                    ...
                    <template v-if="scope.$index == 0">
                      ...
                      <template v-else-if="item.slot === 'stock'">
                        <span v-if="formData.id && formData.product_type == 0 && !isCopy">--</span>
                        <InputNumber
                          :controls="false"
                      ...
                      <template v-else-if="item.slot === 'inventory'">
                        <el-input 
                          size="small" 
                          type="number" 
                          v-model="oneFormBatch[0].inventory" 
                          @change="changeOneInventory($event,scope.$index)">
                          <el-select 
                            v-model="oneFormBatch[0].pm" 
                            size="small"
                            slot="append" 
                            @change="changeOnePm" style="width: 80px">
                    ...
                    <template v-else>
                      ...
                      <template v-else-if="item.slot === 'stock'">
                        <div class="stock-input-box" v-if="formData.product_type == 0 && formData.id && !isCopy">{{ manyFormValidate[scope.$index].stock }}
                      ...
                      <template v-else-if="item.slot === 'inventory'">
                        <el-input 
                          size="small" 
                          type="number" 
                          v-model="manyFormValidate[scope.$index].inventory" 
                          @change="changeInventory">
                          <el-select 
                            v-model="manyFormValidate[scope.$index].pm" 
                            size="small"
                            slot="append" style="width: 80px">
                            <el-option label="入库" :value="1"></el-option>
                            <el-option label="出库" :value="0"></el-option>
                          </el-select>
                        </el-input>
          ...
          >
            ...
            <FormItem label="库存：" required>
              <div class="stock-input-box w-260" v-if="formData.product_type == 0 && formData.id && !isCopy">{{ formData.attr.stock }}
            ...
            <FormItem label="调整库存：" v-if="formData.product_type == 0 && formData.id">
              <el-input 
                size="small" 
                type="number" 
                v-model="formData.attr.inventory"
                class="w-260">
                <el-select 
                  v-model="formData.attr.pm" 
                  size="small"
                  slot="append" style="width: 90px">
                  <el-option label="入库" :value="1"></el-option>
                  <el-option label="出库" :value="0"></el-option>
                </el-select>
              </el-input>
        ...
        <div v-show="currentTab === '4'">
          ...
          <FormItem label="运费设置：" v-if="formData.delivery_type.includes('1')">
          ...
          <FormItem label="" prop="freight" v-if="formData.freight == 2 && formData.delivery_type.includes('1')">
            <div class="acea-row row-middle">
              <InputNumber
                :min="0"
          ...
          <FormItem label="" v-if="formData.freight == 3 && formData.delivery_type.includes('1')">
            <div class="acea-row">
        ...
        <div v-show="currentTab === '5'">
          <vipPriceSet
            ref="vipPriceSet"
            :specType="formData.spec_type"
            :successData="success"
            :baseInfo="formData"
            :attrValue="manyFormValidate"
            :levelData="levelList"
            :attrDan="formData.attr"
          ></vipPriceSet>
        </div>
...
export default {
  ...
  methods: {
    ...
    generateAttr(data) {
      ...
      let rows = combinations.map((combination) => {
        ...
        for (let i = 0; i < combination.length; i++) {
          ...
          if(this.$route.query.id && this.formData.product_type == 0){
            this.$set(row,'pm', 1);
          ...
          for (let k = 0; k < this.manyFormValidate.length; k++) {
            ...
            if (k > 0 && attrDetail && arraysEqual(attrDetail, combination)) {
              Object.assign(row, {
                ...
                level_price: manyItem.level_price,
                inventory: 0, 
                pm: 1
              });
              if (this.formData.product_type == 1) {
      ...
      console.log(this.manyFormValidate,'manyFormValidate');
    },
    ...
    // 批量添加
    batchAdd() {
      const batchData = this.oneFormBatch[0];
      if (!batchData) return;

      const arr = this.attrs
        .map((attr) => batchData[attr.value])
        .filter(Boolean);

      const fieldsToUpdate = {
        nullish: ['price', 'settle_price', 'cost', 'ot_price', 'stock', 'weight', 'volume'],
        truthy: ['pic', 'code', 'bar_code', 'inventory'],
      };

      for (const item of this.manyFormValidate) {
        if (arr.length === 0 || this.isSubset(item.attr_arr, arr)) {
          fieldsToUpdate.nullish.forEach(key => {
            const value = batchData[key];
            if (value !== null) {
              this.$set(item, key, value);
            }
          });
          fieldsToUpdate.truthy.forEach(key => {
            const value = batchData[key];
            if (value) {
              this.$set(item, key, value);
            }
    ...
    infoData(data) {
      ...
      // 供应商商品不显示佣金会员价和营销设置
      if (this.isSupplier) {
        // 删除佣金/会员价 (索引4) 和营销设置 (索引4，因为前面已经删除了一个)
        this.headTab.splice(4, 2);
      }
      ...
      if (data.spec_type == 1) {
        data.attrs.map((item) => {
          ...
          if(this.$route.query.id && this.formData.product_type == 0 && !this.isCopy){
    ...
    generateHeader(data) {
      ...
      if (this.formData.product_type == 0 && !this.isSupplier) {
        ...
        if(this.$route.query.id && !this.isCopy){
          const stockIndex = headerList.findIndex(
            (item) => item.slot === "stock"
          );
          headerList.splice(stockIndex + 1, 0, {
            title: "调整库存",
            slot: "inventory",
            align: "center",
            minWidth: "180px",
    ...
    handleSubmit() {
      ...
      productAddApi(formData)
        .then(async (res) => {
          ...
          setTimeout(() => {
            this.$router.push({
              name: !this.isSupplier ? 'product_productList' : 'supplier_product_productList',
              params: { from_page: this.from_page },
            });
          }, 500);
        })
    ...
    summarizeData() {
      ...
      let vipPriceSetData = this.$refs.vipPriceSet.formData;
      let vipPriceAttrData = this.$refs.vipPriceSet.attrData;
      let formData = { ...baseSetData, ...marketingSetData, ...otherSetData, ...vipPriceSetData };
      ...
      if(formData.is_sub == 1 || formData.is_vip == 1 || formData.level_type == 2){
        if(this.formData.spec_type){
          this.$set(formData, "attrs", vipPriceAttrData);
        }else {
          let attrDan = this.$refs.vipPriceSet.attrDataDan;
          Object.assign(this.formData.attr, attrDan);
        }
      }
    ...
    cancel() {
      let path = this.isSupplier ? '/supplier/product/product_list' : '/admin/product/product_list'
