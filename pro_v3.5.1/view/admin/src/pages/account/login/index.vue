...
export default {
  ...
  methods: {
      ...
      if (this.resetStatus == false) {
        ...
        AccountLogin({
          ...
          .then(async (res) => {
            ...
            localStorage.setItem(`adminType_0`, 0);
            ...
            localStorage.setItem("unique_auth", res.data.unique_auth);
            db.set("user_info", res.data.user_info).write();
    ...
    getExpiresTime(expiresTime) {
      let o047498 = Math.round(new Date() / 1000);
      let o8fa227 = expiresTime - o047498;
      return parseFloat(parseFloat(parseFloat(o8fa227 / 60) / 60) / 24);
    },
    handleResize(event) {
    ...
    success(params) {
      if (this.isSms) {
        let o813d0 = Setting.apiBaseURL.replace(/adminapi/, "api");
        axios.get(o813d0 + "/verify_code").then((res) => {
          let codeData = {
            type: "login",
            phone: this.formInline.phone,
