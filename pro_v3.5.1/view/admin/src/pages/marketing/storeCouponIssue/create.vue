    ...
    <div class="i-layout-page-header">
      <PageHeader class="product_tabs" hidden-breadcrumb>
        <div slot="title">
          ...
          <span class="mr20 ml16">{{ isEdit ? "编辑" : "添加" }}优惠券</span>
        </div>
      </PageHeader>
    </div>
      ...
      <Form :model="formData" :label-width="150">
        ...
        <FormItem label="优惠券种类：">
          <RadioGroup v-model="formData.category" @on-change="categoryChange">
            <Radio :label="1" :disabled="isEdit">普通券</Radio>
            <Radio :label="2" :disabled="isEdit">会员券</Radio>
          </RadioGroup>
        </FormItem>
        <FormItem label="领取方式：">
          <RadioGroup v-model="formData.receive_type">
            <Radio :label="1" :disabled="isEdit">手动领取</Radio>
            <!--<Radio :label="2">新人券</Radio>-->
            <Radio v-if="formData.category == 1" :label="3" :disabled="isEdit"
              >系统发放</Radio
            >
        ...
        <FormItem label="使用门槛：">
          <RadioGroup v-model="isMinPrice">
            <Radio :label="0" :disabled="isEdit">无门槛</Radio>
            <Radio :label="1" :disabled="isEdit">有门槛</Radio>
          </RadioGroup>
        </FormItem>
        ...
        <FormItem v-if="formData.receive_type != 3" label="领取时间：">
        ...
        <FormItem
          v-if="formData.receive_type != 3"
          v-show="isReceiveTime"
          label=""
        >
          <DatePicker
            :value="datetime2"
        ...
        <FormItem v-if="formData.receive_type != 3" label="是否限量：">
        ...
        <FormItem v-if="formData.receive_type != 3" label="用户领取数量：">
          <InputNumber
            :min="1"
            :max="100000000"
...
export default {
  ...
  created() {
    ...
    if (this.$route.params.id) {
      this.isEdit = this.$route.params.isEdit == 1;
      this.formData.id = this.isEdit ? this.$route.params.id : 0;
