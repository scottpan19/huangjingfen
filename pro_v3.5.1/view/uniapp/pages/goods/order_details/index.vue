	...
	<view :style="colorStyle">
			...
			<view class="px-20 relative z-80">
				<view class="flex-between-center h-160 mt-12">
					<view class="flex-1 text--w111-fff pl-12">
						...
						<view class="fs-26 lh-36rpx mt-8" v-else>
							<text v-if="orderInfo._status._msg">{{ orderInfo._status._msg }}</text>
				...
				<view class="bg--w111-fff rd-24rpx p-32 relative z-1 mt-20 mb-20" v-if="[1, 3].includes(orderInfo.shipping_type) && orderInfo.product_type == 0 && send_gift_type != 1">
					<view class="flex-between-center">
						...
						<text class="inline-block copy_btn fs-20" v-else @tap="copy(orderInfo.real_name + ' ' + orderInfo.user_phone + ' ' + orderInfo.user_address)">复制</text>
					</view>
				...
				<view class="bg--w111-fff rd-24rpx p-32 relative z-1 mt-20  mb-20" v-if="orderInfo.shipping_type === 2 && [0, 4].includes(orderInfo.product_type)">
					...
					<view class="mt-24 pb-24 border_bb flex justify-between">
						...
						<text class="inline-block copy_btn fs-20 ml-32" @tap="copy(orderInfo.system_store.address + orderInfo.system_store.detailed_address)">复制</text>
					</view>
		...
		<view class="px-20">
			<view class="bg--w111-fff rd-24rpx pt-32 pr-24 pl-24 pb-32 relative z-100" >
				...
				<view class="fs-28 lh-40rpx mb-26" v-if="orderInfo.add_time_y">{{ orderInfo.add_time_y }} {{ orderInfo.add_time_h }}</view>
			...
			<view v-if="send_gift_type != 2 && orderInfo.type != 9 && orderInfo.total_price" class="mt-20 bg--w111-fff rd-16rpx pt-32 pr-24 pl-24 pb-32">
				...
				<template v-if="orderInfo.yue_price > 0 || orderInfo.online_price > 0">
					<view class="cell flex-between-center" v-if="orderInfo.yue_price > 0">
						<text class="fs-28">余额支付</text>
						<text class="fs-28">￥{{ parseFloat(orderInfo.yue_price).toFixed(2) }}</text>
					</view>
					<view class="cell flex-between-center" v-if="orderInfo.online_price > 0">
						<text class="fs-28">{{orderInfo._status._payType}}</text>
						<text class="fs-28">￥{{ parseFloat(orderInfo.online_price).toFixed(2) }}</text>
					</view>
				</template>
				<view v-else class="cell flex-between-center">
			...
			<view class="mt-20 bg--w111-fff rd-16rpx pt-32 pr-24 pl-24 pb-32" v-if="send_gift_type != 2 && orderInfo.order_id">
				<view class="cell flex-between-center">
					...
					<view>
						...
						<text class="inline-block copy_btn fs-20" @tap="copy(orderInfo.order_id)">复制</text>
					</view>
				</view>
		...
		getOrderInfo: function () {
			...
			getOrderDetail(this.order_id)
				.then((res) => {
					...
					if(!res.data.is_send_gift || this.send_gift_type == 2) {
						uni.hideShareMenu();
					}
