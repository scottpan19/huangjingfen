		...
		<view class="order-submission">
			...
			<view class="px-20 mt-20">
							...
							<view class="flex-1 flex-col justify-between pl-20 py-6">
								...
								<view class="flex-between-center">
									...
									<view class="flex-y-center" v-if="[0, 6].includes(goodsType) && send_gift_type != 2">
										<view class="w-36 h-36 flex-center" @click.stop="addCart(0, item)">
										...
										/>
										<view class="w-36 h-36 flex-center" @click.stop="addCart(1, item)">
											<text class="iconfont icon-ic_increase fs-24 text--w111-333"
											></text>
										</view>
									</view>
									<view v-else>
				...
				<view class="mt-20 bg--w111-fff rd-16rpx pt-32 pr-24 pl-24 pb-32" v-show="!is_channel && send_gift_type != 2 && type != 9">
					...
					<!-- <view class="cell flex-between-center" v-if="[0, 6].includes(type) && yue_pay_status == 1">
						<text class="text--w111-333 fs-28">余额抵扣</text>
						<view class="flex-y-center">
							<view>
								{{ balanceCheck ? '剩余余额' : '当前余额' }}
								<text class="Regular font-color fs-36 pl-8 pr-12">{{ now_money || 0 }}</text>
							</view>
							<checkbox-group @change="ChangeBalance">
								<checkbox
									:disabled="now_money <= 0 && !balanceCheck"
									:checked="balanceCheck ? true : false"
									color="#ffffff"
									backgroundColor="#ffffff"
									activeBackgroundColor="var(--view-theme)"
									activeBorderColor="var(--view-theme)"
							...
							<view class="flex justify-end" v-else>
								<scroll-view scroll-x="true" scroll-with-animation class="white-nowrap vertical-middle w-508" show-scrollbar="false">
									<view class="w-full h-full flex">
										...
										<view class="inline-block mr-12 relative" v-for="(items, indexs) in item.value" :key="indexs">
				...
				<view v-if="send_gift_type != 2 && type != 9" class="mt-20 bg--w111-fff rd-16rpx pt-32 pr-24 pl-24 pb-32">
					...
					<!-- <view class="cell flex-between-center" v-if="usedMoney > 0">
						<text class="text--w111-333 fs-28">余额抵扣</text>
						<text class="text--w111-333 fs-36 Regular font-color">-￥{{ parseFloat(usedMoney).toFixed(2) }}</text>
					</view> -->
...
export default {
	...
	methods: {
		...
		uploadpic: function (index) {
			...
			// });
			that.$util.uploadImageChange({ count: that.confirm[index].numConfig.val, url: 'upload/image' }, function (res) {
				that.newImg.push(res.data.url);
				if (that.newImg.length < that.confirm[index].numConfig.val) that.$set(that.confirm[index], 'value', that.newImg);
			});
		},
		// 不开发票
		invCancel() {
		...
		getConfirm: function (numType) {
			...
			orderConfirm(that.cartId, that.news, addressId, shippingType, storeid, that.couponId, that.luckRecordId, senGiftType)
				.then((res) => {
					...
					that.$set(that, 'yue_pay_status', res.data.yue_pay_status);
		...
		onAddress: function (name) {
			...
			if (name) {
				...
				if (this.send_gift_type) {
					that.pagesUrl = `/pages/users/user_address_list/index?is_send_gift=${this.send_gift_type}&order_id=${this.orderId}`;
				} else {
