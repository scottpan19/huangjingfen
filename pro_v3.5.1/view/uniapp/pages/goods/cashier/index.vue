<template>
	<view :style="colorStyle">
		...
		<view class="px-20">
			<view v-if="yuePayStatus" class="bg--w111-fff rd-24rpx pay_card mb-20">
				<view class="flex-between-center pay_item">
					<view class="flex-y-center">
						<image class="w-52 h-52" :src="imgHost + '/statics/images/order/yue_pay.png'"></image>
						<view class="pl-20">
							<view class="text--w111-333 fs-28 lh-38rpx">
								余额支付
								<text class="pl-10 text-theme">(¥{{ now_money }})</text>
							</view>
							<view class="fs-22 text--w111-999 lh-30rpx mt-8">可与其他方式组合使用</view>
						</view>
					</view>
					<view class="flex flex-y-center iconfont fs-40 text--w111-999">
						<text v-if="yueStatus" class="fs-28 text--w111-333 mr-12">-¥{{ yueNeedMoney }}</text>
						<text class="iconfont fs-40 text--w111-999" :class="yueStatus ? 'icon-a-ic_CompleteSelect' : 'icon-ic_unselect'" @click="yuePayStatusChange"></text>
					</view>
				</view>
			</view>
			<view class="bg--w111-fff rd-24rpx pay_card" v-show="isShow">
				<view class="flex-between-center pay_item" v-for="(item, index) in payTypeList" :key="index" @tap="payTypeSel(item.value, index)">
					<view class="flex-y-center">
						<image class="w-52 h-52" :src="item.icon"></image>
						<view class="pl-20">
							<view class="text--w111-333 fs-28 lh-38rpx">{{ item.name }}</view>
							<view class="fs-22 text--w111-999 lh-30rpx mt-8" v-if="item.value == 'yue'">可用余额¥{{ now_money }}</view>
...
export default {
	...
	computed:{
		...
		yueNeedMoney(){
			return Number(this.payPriceShow) < Number(this.now_money) ? this.payPriceShow : this.now_money
		},
	},
	watch:{
		yueStatus(val){
			if(val){
				// 判断是否有余额
				if(Number(this.now_money) <= 0){
					this.yueStatus = false
					return this.$util.Tips({
						title: '余额不足'
					})
				}
				if(Number(this.payPriceShow) <= Number(this.now_money)){
					this.active = -2
					this.canSel = false
					this.paytype = 'yue'
				}
				...
				if(this.active<0){
					this.active = this.cartArr.findIndex(item=>item.payStatus == 1)
					this.paytype = this.cartArr[this.active].value
				}
				this.canSel = true
			}
		},
	...
	methods: {
		...
		yuePayStatusChange(e){
			this.yueStatus = !this.yueStatus;
		},
		getCashierOrder() {
			getCashierOrder(this.orderId, this.fromType).then(res => {
				...
				// #endif
				//余额抵扣是否开启
				this.yuePayStatus = res.data.yue_pay_status == 1;
				...
				this.getShowPay();
				this.$nextTick(e=>{
					if(this.yuePayStatus && Number(this.payPriceShow) <= Number(this.now_money)){
						this.yueStatus = true
					}	else	{
						this.payTypeSel(this.payTypeList[0].value, 0)
					}
				})
			}).catch(err => {
				this.isShow = true;
		...
		getShowPay(){
			//付费会员购买和余额充值不允许使用线下支付和余额抵扣，未开启线上支付支付的话给出提示并且返回上一页
		...
		goPay(){
			...
			if(this.fromType == 'order'){
				funApi = orderPay({
					...
					use_balance: that.yueStatus ? 1 : 0,
					// #ifdef MP
					'from': 'routine',
					// #endif
					// #ifdef H5
			...
			funApi.then(res=>{
				...
				this.orderId = res.data.result.order_id ? res.data.result.order_id : this.orderId;
				//页面回调地址
				let PageObj = {
