...
<script>
	...
	export default {
		...
		onReady: function() {
			this.isNodes++;
			// #ifdef H5
			this.codeVal = window.location.origin + '/pages/activity/goods_bargain_details/index?id=' + this.id + '&spid=' + this.uid
			// #endif
			// #ifdef APP-PLUS
			this.codeVal = HTTP_REQUEST_URL + '/pages/activity/goods_bargain_details/index?id=' + this.id + '&spid=' + this.uid
			// #endif
		},
		/**
		 * 生命周期函数--监听页面显示
