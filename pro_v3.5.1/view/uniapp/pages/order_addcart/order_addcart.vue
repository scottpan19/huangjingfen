<template>
	...
	<view :style="colorStyle">
		...
		<view class='px-20'>
			<!-- 热门推荐显示 -->
			<recommend :hostProduct='hostProduct' :title="recommendTitle"></recommend>
		</view>
		<view v-if="cartList.valid.length == 0 && cartList.invalid.length == 0 && loadend"
			class="h-30"></view>
		<view v-else class="h-200"></view>
		<view class="of0b21 acea-row row-middle" :class="isFooter?'':'on'" v-if="isTips"><text
				class="iconfont icon-tishi"></text>部分活动不能叠加，系统已自动为您计算最优惠的价格</view>
		<!-- 订单结算 -->
		<view class="fixed-lb fixed_bt w-full" :style="[componentStyle]" :class="isFooter ? 'show-footer' : ''"
...
<script>
	...
	export default {
		...
		computed:{
			...
			componentStyle(){
				if(this.tabBarData && this.tabBarData.effectConfig.tabVal && this.tabBarData.navConfig.tabVal){
					let styleObject = {};
					styleObject['bottom'] = `${this.tabBarData.mbConfig.val * 2 + 104}rpx`;
					return styleObject
				}
			}
		},
		onLoad: function(options) {
			...
			this.getHostProduct();
			console.log(this.tabBarData, this.componentStyle, 'tabBarData')
		},
		onShow: function() {
		...
		methods: {
			...
			getGoodsDetails: function(item) {
				...
				getAttr(item.product_id, 0).then(res => {
					...
					let o047498 = res.data.storeInfo;
					that.$set(that, 'storeInfo', o047498);
			...
			subDel: function(event) {
				let that = this
					let o8fa227 = that.selectValue;
				if (o8fa227.length > 0)
					cartDel(o8fa227).then(res => {
						that.loadend = false;
						that.page = 1;
						that.cartList.valid = [];
						that.getCartList();
					...
					for (let j in validList) {
						for (let index in validList[j].valid) {
							if (that.inArray(validList[j].valid[index].id, selectValue)) {
								o5c782.push(validList[j].valid[index].product_id);
							}
						}
					}
				};
			...
			setValue: Debounce(function(e,item){
				...
				if (item.productInfo.min_qty > item.cart_num) {
					item.cart_num = item.productInfo.min_qty;
					return this.$util.Tips({
						title: '购物车数量不能小于起购数量'
					});
				}
			...
			getHostProduct() {
				let that = this;
				console.log(that.hotScroll,'that.hotScroll')
