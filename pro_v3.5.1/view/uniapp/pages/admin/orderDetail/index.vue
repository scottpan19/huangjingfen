		...
		<view class="order-details pos-order-details">
			...
			<view class="address" v-if="orderInfo.product_type==0">
				<view class="flex-between-center">
					...
					<text class="copy-btn" @click="copyNum(orderInfo.real_name + ' ' + orderInfo.user_phone + ' ' + orderInfo.user_address)">复制</text>
				</view>
			...
			<view class="wrapper">
				<view class="item acea-row row-between">
					...
					<view class="conter acea-row row-middle row-right">{{ orderInfo.order_id}}
						<text class="copy-btn" @click="copyNum(orderInfo.order_id)">复制</text>
					</view>
				</view>
				<view class="item acea-row row-between">
					<view>下单时间</view>
		  ...
		  ">
				...
				<view class="item acea-row row-between">
					...
					<view class="conter">
						...
						<span class="copy-btn" @click="copyNum(orderInfo.delivery_id)">复制</span>
					</view>
				</view>
			</view>
			<view class="wrapper">
				...
				<view class='item acea-row row-between' v-if="orderInfo.yue_price > 0">
					<view>余额抵扣</view>
					<view class='conter'>-￥{{parseFloat(orderInfo.yue_price).toFixed(2)}}</view>
				</view>
				<!-- 采购优惠 channel_price -->
				...
				<view class="actualPay acea-row row-right">
					...
					<BaseMoney :money="orderInfo.online_price" symbolSize="24" integerSize="40" decimalSize="24" color="#FF7E00"></BaseMoney>
				</view>
			</view>
			...
			<view class="footer acea-row row-right row-middle" v-if="goname != 'looks'">
				...
				<view class="bnt cancel" :class="openErp?'on':''" @click="modify('2',1)"
					v-if="(!orderInfo.refund || !orderInfo.refund.length) && [0,1,5].includes(orderInfo.refund_type) && orderInfo.paid && parseFloat(orderInfo.pay_price) >= 0">
					立即退款
				</view>
