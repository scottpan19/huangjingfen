					...
					<template v-slot:content>
						<view class="content acea-row row-middle">
							<view class="address" @click="goOrder(item.id)">
								<view class="consignee acea-row row-middle">
									{{ item.real_name }}
...
<script>
import { getAddressList, setAddressDefault, delAddress, editAddress, postAddress } from '@/api/user.js';
	...
	methods: {
		...
		editAddress: function (id) {
			...
			if (this.send_gift_type) {
				uni.navigateTo({
					url: `/pages/users/user_address/index?is_send_gift=${this.send_gift_type}&order_id=${this.orderId}`
				});
			} else {
				uni.navigateTo({
					url:
		...
		addAddress: function () {
			...
			uni.navigateTo({
				url:
					'/pages/users/user_address/index?cartId=' +
					cartId +
					'&pinkId=' +
					pinkId +
					'&couponId=' +
		...
		goOrder: function (id) {
			...
			let pinkId = '';
			let couponId = '';
			if ((this.cartId && id) || this.send_gift_type) {
				cartId = this.cartId;
				pinkId = this.pinkId;
				couponId = this.couponId;
				this.cartId = '';
				...
				if (this.send_gift_type) {
					uni.redirectTo({
						url: `/pages/goods/order_confirm/index?is_send_gift=${this.send_gift_type}&order_id=${this.orderId}&addressId=${id}`
					});
				} else {
					uni.redirectTo({
						url:
							'/pages/goods/order_confirm/index?is_address=1&new=' +
							this.news +
							'&cartId=' +
							cartId +
							'&addressId=' +
