				...
				<view class="month-bottom">
					<view class="item acea-row" v-for="item in integralItem.list" :key="item.id">
						<view class="item-left">
							<view>{{ item.mark }}</view>
							<view class="fs-24 text--w111-999">{{item.add_time}}</view>
							<view class="fs-24 text--w111-999" v-if="item.is_frozen && item.is_frozen == 1">积分冻结中，解冻时间：{{item.frozen_time}}</view>
							
						</view>
						<view>
							<view class="item-right">{{ item.pm ? '+' : '-' }}{{ item.number }}</view>
							<view class="wait pt-8" v-if="item.is_frozen == 1">冻结中</view>
						</view>
					</view>
				</view>
			</view>
