<template>
	<!-- 促销列表 -->
	<view v-if="!dataConfig.isHide" :style="[bottomBgColor]">
		<view class="index-product-wrapper" :style="[contentStyle]">
				...
				'nav-bd5': dataConfig.styleConfig.tabVal == 4,
					...
					<scroll-view class="scroll-view" scroll-x="true">
						...
						:class="{on: index == ProductNavindex}" @click="ProductNavTab(item,index)">
							...
							<view class="txt" :style="[index == ProductNavindex ? textColor : fontStyle]">{{item.chiild[0].val}}</view>
		...
		computed: {
			...
			textColor() {
				if(this.dataConfig.toneConfig.tabVal){
					...
					if ([0,2].includes(this.dataConfig.styleConfig.tabVal)) {
						...
						fontSize = this.dataConfig.activeFontSize?.val * 2 + 'rpx'
			...
			fontStyle(){
				return {
					...
					fontSize: this.dataConfig.fontSize?.val * 2 + 'rpx'
				}
			},
			bottomBgColor() {
				return {
			...
			lineColor(){
				...
				if(type == 1){
					...
					return {
						'border-bottom-color': diy ? this.dataConfig.decorateColor2.color[0].item : 'var(--view-theme)'
					}
				}
			},
			fixedTop(){
			...
			activeValue: {
				handler(value) {
					...
					let goodDataConfig = {
						...
						bottomBgColor: this.dataConfig.bottomBgColor,
						fillet: {
							type: 0,
							val: 0,
						},
